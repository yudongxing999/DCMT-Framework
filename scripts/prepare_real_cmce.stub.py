#!/usr/bin/env python3
"""Loads prepare_real_cmce.full.py if present; else prepare_real.part*.b64 / payload.b64 (gzip) with CRC32."""
import gzip, base64, zlib
from pathlib import Path
_here = Path(__file__).resolve().parent
_full = _here / "prepare_real_cmce.full.py"
if _full.exists():
    exec(compile(_full.read_text(), str(_full), "exec"))
else:
    _parts = sorted(_here.glob("prepare_real.part*.b64"), key=lambda p: int(p.stem.split("part")[-1]))
    if _parts:
        _b64 = "".join(p.read_text().strip() for p in _parts)
    else:
        _mono = _here / "prepare_real.payload.b64"
        if not _mono.exists():
            raise FileNotFoundError("missing prepare_real_cmce.full.py / prepare_real.part*.b64 / payload.b64")
        _b64 = _mono.read_text().strip()
    _raw = gzip.decompress(base64.b64decode(_b64))
    _expect_crc = 3883616231
    _got = zlib.crc32(_raw) & 0xffffffff
    if _got != _expect_crc:
        raise RuntimeError(f"prepare_real payload CRC mismatch: {_got:08x} != {_expect_crc:08x}")
    exec(compile(_raw.decode(), str(Path(__file__).resolve()), "exec"))
