...
def make_sample(idx: int, rng: random.Random, max_seq: int, num_labels: int) -> Dict[str, Any]:
    ...
    # Stable label from phrase content + idx (avoid PYTHONHASHSEED nondeterminism).
...
def main():
    ...
    train = [make_sample(i, rng, args.max_seq, args.num_labels) for i in range(args.n_train)]
    val = [
        make_sample(args.n_train + i, rng, args.max_seq, args.num_labels)
